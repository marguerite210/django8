...
#제네릭뷰: jango에서 제공하는 여러가지 기능을 하는 뷰클래스
#class 뷰이름(제네릭뷰):
...
class index(ListView):
    ...
    #paginate_by: 한 페이지에 몇 개의 객체가 보여질지 설정하는 변수
    paginate_by = 5
...
def searchP(request):
    ...
    #'0': 제목검색
    if t == '0':
        #Post.objects.filter(조건): 특정조건을 만족하는 Post 객체 추출
        #filter, exclude, get함수 사용시: 모델클래스 내에서 정의한 변수명__명령어 = 값
        #contains: 우변값에 해당하는 문자열이 해당 객체의 변수에 저장되어있는지 확인
